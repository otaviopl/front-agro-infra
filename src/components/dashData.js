...
const formatChartData = (data, label, color) => ({
  labels: data.map((item) => `${item.horas}h`),
  datasets: [
    {
      label: label,
    ...
    <Box sx={{ marginTop: "2rem" }}>
      ...
      <Grid container spacing={3} sx={{ marginTop: "1rem" }}>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ padding: 2, backgroundColor: "#1e1e1e", color: "#e0e0e0" }}>
            <Typography variant="subtitle1">Temperatura Máxima (°C)</Typography>
            <Line
              data={formatChartData(data.tmax2m, "Temperatura Máxima", "rgba(255,99,132,1)")}
            />
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ padding: 2, backgroundColor: "#1e1e1e", color: "#e0e0e0" }}>
            ...
            <Line
              data={formatChartData(data.sunsdsfc, "Horas de Sol", "rgba(54,162,235,1)")}
            />
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ padding: 2, backgroundColor: "#1e1e1e", color: "#e0e0e0" }}>
            ...
            <Line
              data={formatChartData(data.soill0_10cm, "Umidade do Solo", "rgba(75,192,192,1)")}
            />
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ padding: 2, backgroundColor: "#1e1e1e", color: "#e0e0e0" }}>
            ...
            <Line
              data={formatChartData(data.tmin2m, "Temperatura Mínima", "rgba(153,102,255,1)")}
            />
          </Paper>
        </Grid>
      </Grid>
